                        ...
                        <div class="card-body">
                            <form action="{{ route('admin.articles.update', $article->ID) }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="row mb-4">
                                    <div class="col-md-8">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control @error('TieuDe') is-invalid @enderror" id="TieuDe" name="TieuDe" value="{{ old('TieuDe', $article->TieuDe) }}" placeholder="Nhập tiêu đề bài viết...">
                                            @error('TieuDe')
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <textarea class="form-control @error('MoTaNgan') is-invalid @enderror" id="MoTaNgan" name="MoTaNgan" rows="3" placeholder="Tóm tắt nội dung bài viết...">{{ old('MoTaNgan', $article->MoTaNgan) }}</textarea>
                                            @error('MoTaNgan')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                                    ...
                                    <div class="col-md-4">
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select @error('IDDanhMuc') is-invalid @enderror" id="IDDanhMuc" name="IDDanhMuc">
                                                ...
                                                @foreach($categories as $cat)
                                                    <option value="{{ $cat->ID }}" {{ old('IDDanhMuc', $article->IDDanhMuc) == $cat->ID ? 'selected' : '' }}>{{ $cat->TenDanhMuc }}</option>
                                                @endforeach
                                            </select>
                                            @error('IDDanhMuc')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select @error('TrangThai') is-invalid @enderror" id="TrangThai" name="TrangThai">
                                                <option value="1" {{ old('TrangThai', $article->TrangThai) == '1' ? 'selected' : '' }}>Hiển thị</option>
                                                <option value="0" {{ old('TrangThai', $article->TrangThai) == '0' ? 'selected' : '' }}>Ẩn</option>
                                            </select>
                                            @error('TrangThai')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <div class="preview-image" id="imagePreview">
                                                @if($article->HinhAnh)
                                                    <img src="{{ asset($article->HinhAnh) }}" alt="Preview">
                                                @else
                                ...
                                <div class="mb-4">
                                    ...
                                    <textarea class="form-control @error('NoiDung') is-invalid @enderror" id="NoiDung" name="NoiDung" rows="10">{{ old('NoiDung', $article->NoiDung) }}</textarea>
                                    @error('NoiDung')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="d-flex justify-content-end gap-3">
                                    ...
                                    <button type="submit" class="btn-submit">
                                        <i class="bx bx-save me-2"></i> Cập nhật bài viết
                                    </button>
                                </div>
                            </form>
                        </div>
    ...
    <script>
        function previewImage(input) {
            ...
            if (input.files && input.files[0]) {
                ...
                // If no new file selected, keep showing old image if exists
                @if($article->HinhAnh)
                    preview.innerHTML = `<img src="{{ asset($article->HinhAnh) }}" alt="Preview">`;
                @else
